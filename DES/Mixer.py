...
class Mixer:
    ...
    def encrypt(self, binary: str) -> str:
        ...
        # applying function
        r2: str = Util.int_to_bin(self.func(int(r1, base=2), self.key), block_size=self.initial_permutation.out_degree)
        ...
        r3: str = self.final_permutation.permutate(r3)
         ...
         # applying xor
        l = Util.int_to_bin(int(l, base=2) ^ int(r3, base=2), block_size=self.block_size // 2)
        return l + r
